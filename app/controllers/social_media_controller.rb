class SocialMediaController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @social_medium.save
        format.html { redirect_to social_medium_url(@social_medium), notice: "Social medium was successfully created." }
  ...
  def update
    respond_to do |format|
      if @social_medium.update(social_medium_params)
        format.html { redirect_to social_medium_url(@social_medium), notice: "Social medium was successfully updated." }
  ...
  private
    ...
    def social_medium_params
      params.require(:social_medium).permit(:uid, :name, :url, :user_id)
    end
end
